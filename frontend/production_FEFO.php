            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 gap-4">
                <div>
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="text-red-500 text-sm font-bold uppercase tracking-wider"><?= __('critical_alert_level') ?></span>
                    </div>
                    <h1 class="text-3xl font-bold text-[#10b981] mb-2"><?= __('fefo_red_alerts') ?></h1>
            ...
            <div class="bg-[#0f1722] rounded-lg border border-[#1f2937] overflow-hidden">
                <div class="p-5 border-b border-[#1f2937] flex justify-between items-center bg-[#0b121c]">
                    <div class="flex items-center gap-3">
                        <h3 class="text-[#10b981] font-bold"><?= __('expiring_batches_list') ?></h3>
                        <span class="bg-[#b91c1c] text-white text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded"><?= __('active_alerts') ?></span>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-[#0a1118] text-[11px] uppercase tracking-wider text-gray-500 border-b border-[#374151]">
                                <th class="p-4 font-bold"><?= __('batch_id') ?></th>
                                <th class="p-4 font-bold"><?= __('product_name') ?></th>
                                <th class="p-4 font-bold"><?= __('expiry_date') ?></th>
                                <th class="p-4 font-bold"><?= __('current_quantity') ?></th>
                                <th class="p-4 font-bold"><?= __('storage_zone') ?></th>
                            ...
                            <?php else: ?>
                                <?php foreach ($expiringBatches as $batch): ?>
                                    <tr class="hover:bg-[#131c26] transition-colors border-l-2 border-transparent hover:border-red-500">
                                        ...
                                        <td class="p-4 font-mono text-red-300 text-xs">
            ...
            <form action="../backend/connection/process_allocation.php" method="POST" class="p-6">
                ...
                <div class="space-y-3 mb-6 text-sm">
                    <div class="flex justify-between border-b border-[#1f2937]/50 pb-2">
                        <span class="text-gray-500"><?= __('target_batch_id') ?>:</span>
                        <span class="text-[#10b981] font-mono font-bold bg-[#10b981]/10 px-2 py-0.5 rounded" id="display_batch_id">--</span>
                    </div>
                    <div class="flex justify-between border-b border-[#1f2937]/50 pb-2">
                        <span class="text-gray-500"><?= __('max_stock_available') ?>:</span>
                        <span class="text-red-300 font-mono font-bold" id="display_max_stock">0.00 kg</span>
                    </div>
                </div>
                
                <div class="mb-8">
                    <label class="block text-xs text-gray-400 uppercase tracking-wider mb-2 font-bold"><?= __('allocation_quantity') ?> (kg)</label>
                    <div class="relative">
                    ...
                    <p class="text-[10px] text-yellow-500 mt-2 font-medium">⚠️ <?= __('note_letters_restricted') ?></p>
                </div>
                
                <div class="flex gap-3">
                    <button type="button" onclick="closeModal()" class="flex-1 bg-transparent hover:bg-[#1f2937] border border-[#374151] text-gray-300 text-sm font-bold py-3 rounded transition-colors"><?= __('cancel') ?></button>
                    <button type="submit" class="flex-1 bg-[#10b981] hover:bg-[#059669] text-gray-900 text-sm font-bold py-3 rounded transition-colors shadow-[0_0_15px_rgba(16,185,129,0.3)]"><?= __('allocate_now') ?></button>
                </div>
            </form>
        </div>
    </div>
