...
try {
    ...
    $batch = $controller->handlePerformScreen($batch_id, $lang);
} catch (Exception $e) {
...
<body class="min-h-screen overflow-x-hidden">
    ...
    <main class="md:ml-64 p-6 md:p-8 pt-24 md:pt-8 w-full md:w-[calc(100%-256px)]">
        ...
        <div class="mb-6 pb-4 border-b border-[#1f2937]">
            <h1 class="text-2xl font-bold text-white tracking-wide"><?= __('batch_inspection') ?><?= htmlspecialchars($batch['BCH_batch_id']) ?></h1>
            <p class="text-xs text-[#10b981] uppercase font-bold tracking-widest mt-1 flex items-center gap-2">
                ...
                <?= __('active_qc_protocol_execution') ?>
            </p>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="space-y-6">
                    ...
                    <div class="bg-[#0f1722] p-5 rounded-lg border border-[#1f2937]">
                        <h3 class="text-sm font-bold text-white mb-4 uppercase tracking-wider"><?= __('material_identification') ?></h3>
                        <div class="space-y-3 text-xs">
                            <div class="flex justify-between border-b border-[#1f2937]/50 pb-2">
                                <span class="text-gray-500"><?= __('material_grade_colon') ?></span>
                                <span class="text-[#10b981] font-mono font-bold"><?= htmlspecialchars($batch['PRD_material_grade'] ?? __('standard')) ?></span>
                            </div>
                            <div class="flex justify-between border-b border-[#1f2937]/50 pb-2">
                                <span class="text-gray-500"><?= __('batch_id_colon_perform') ?></span>
                            ...
                            <div class="flex justify-between border-b border-[#1f2937]/50 pb-2">
                                ...
                                <span class="text-gray-300"><?= htmlspecialchars($batch['SUP_origin_facility'] ?? __('inbound_terminal')) ?></span>
                            </div>
                            <div class="flex justify-between pb-1">
                                <span class="text-gray-500"><?= __('shipment_date_colon') ?></span>
                    ...
                    <div class="bg-[#0f1722] p-5 rounded-lg border border-[#1f2937]">
                        ...
                        <div class="border border-dashed border-[#374151] hover:border-[#10b981] rounded-lg p-1 text-center cursor-pointer transition-colors relative h-48 flex items-center justify-center bg-[#07121a] group">
                            ...
                            <div id="enlargeOverlay" class="hidden absolute inset-0 bg-black/60 backdrop-blur-sm rounded-lg z-30 flex flex-col items-center justify-center cursor-zoom-in pointer-events-none group-hover:flex transition-all opacity-0 group-hover:opacity-100">
                                ...
                                <span class="text-xs font-bold text-white tracking-widest"><?= __('click_to_enlarge') ?></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-[#0f1722] p-6 rounded-lg border border-[#1f2937] space-y-6">
                        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-2 border-b border-[#1f2937] pb-4">
                            <h3 class="text-sm font-bold text-white uppercase tracking-wider"><?= __('qc_quality_protocol') ?></h3>
                            <h3 class="text-xs text-gray-400 bg-[#1f2937]/50 px-3 py-1.5 rounded font-mono">
                                <?= __('protocol_colon') ?> <strong class="text-white"><?= __('iso_standard') ?></strong>
                            </h3>
                            <span class="text-xs text-gray-400 bg-[#1f2937]/50 px-3 py-1.5 rounded font-mono">
                                <?= __('phase_quantitative') ?>
                            </span>
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-[11px] text-[#10b981] uppercase font-bold mb-2 tracking-wider"><?= __('received_quantity') ?></label>
                            ...
                            <div>
                                <label for="rejected_qty" class="block text-[11px] text-[#10b981] uppercase font-bold mb-2 tracking-wider"><?= __('enter_rejected_quantity') ?></label>
                        ...
                        <div>
                            <label for="inspector_comments" class="block text-[11px] text-gray-400 uppercase font-semibold mb-2"><?= __('inspector_comments') ?></label>
                            <textarea id="inspector_comments" name="inspector_comments" rows="3" placeholder="<?= __('log_compliance_comments') ?>"
                        ...
                        <div class="mt-8 pt-6 border-t border-[#1f2937]">
                            <div class="p-4 bg-[#07121a] rounded border border-[#1f2937] flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                                <div>
                                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wide"><?= __('calculated_usable_yield') ?></p>
                                    <p class="text-[11px] text-gray-500 mt-1"><?= __('est_pass_threshold') ?></p>
                                </div>
    ...
    <div id="lightbox" class="hidden fixed inset-0 bg-black/95 z-[60] flex flex-col items-center justify-center p-4 backdrop-blur-md transition-opacity" onclick="closeLightbox()">
        ...
        <p class="text-gray-400 font-mono text-xs mt-4"><?= __('visual_evidence_record') ?></p>
    </div>
