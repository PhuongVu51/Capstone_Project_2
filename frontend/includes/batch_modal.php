...
try {
    if (isset($pdo)) {
        $stmt_z = $pdo->query("SELECT STZ_zone_id, STZ_zone_name FROM STORAGE_ZONES");
...
<div id="batch-detail-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center hidden transition-all duration-300">
    <div class="bg-[#0e1b2a] border border-[#1f2937] rounded-xl p-6 max-w-2xl w-full mx-4 shadow-2xl overflow-y-auto max-h-[90vh]">
        <div class="flex justify-between items-center pb-4 mb-4 border-b border-[#1f2937]">
            <div>
                <span class="text-xs uppercase tracking-wider text-[#10b981] font-bold">Chi tiết Lô hàng</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-6">
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Mặt hàng / Sản phẩm</p>
            ...
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Nhà cung cấp</p>
                <p id="bd-supplier-name" class="text-white font-bold mt-1 text-base">-</p>
            </div>
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Tồn kho hiện tại / Ban đầu</p>
                <p class="mt-1">
            ...
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Khu vực kho</p>
                <p id="bd-zone-name" class="text-white font-bold mt-1 text-base">-</p>
            </div>
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Ngày nhập kho</p>
                <p id="bd-received-date" class="text-gray-200 mt-1 font-mono">-</p>
            </div>
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Ngày hết hạn</p>
                <p id="bd-expiry-date" class="text-amber-400 font-bold mt-1 font-mono">-</p>
            </div>
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Trạng thái sức khỏe</p>
                <span id="bd-health-status" class="inline-block mt-1 px-2.5 py-0.5 rounded text-xs font-bold uppercase">-</span>
            </div>
            <div class="bg-[#07121a] p-3.5 rounded-lg border border-[#1f2937]">
                <p class="text-xs text-gray-400 font-semibold uppercase">Giai đoạn kiểm định (QC Stage)</p>
...
<div id="batch-edit-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center hidden transition-all duration-300">
    <div class="bg-[#0e1b2a] border border-[#1f2937] rounded-xl p-6 max-w-lg w-full mx-4 shadow-2xl">
        <div class="flex justify-between items-center pb-4 mb-4 border-b border-[#1f2937]">
            <div>
                <span class="text-xs uppercase tracking-wider text-amber-400 font-bold">Chỉnh sửa thông tin Lô hàng</span>
        ...
        <form action="../backend/controllers/StockController.php?action=update_batch_full" method="POST" id="be-form" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Khu vực lưu trữ (Storage Zone)</label>
            ...
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Ngày hết hạn (Expiry Date)</label>
            ...
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Tồn kho khả dụng (kg)</label>
            ...
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Trạng thái sức khỏe (Health Status)</label>
                <select name="health_status" id="be-health-status" class="w-full bg-[#07121a] border border-[#374151] text-white rounded-lg p-2.5 text-sm focus:border-[#10b981] focus:outline-none">
                    ...
                    <option value="Critical">Critical (Nguy cơ hỏng)</option>
                </select>
            </div>
    ...
    fetch('../backend/controllers/StockController.php?action=get_batch_detail&batch_id=' + encodeURIComponent(batchId))
        ...
        .then(res => {
            if (res.success && res.data) {
                ...
                document.getElementById('bd-material-grade').innerText = b.PRD_material_grade ? 'Phân loại: ' + b.PRD_material_grade : '';
                document.getElementById('bd-supplier-name').innerText = b.SUP_supplier_name || '-';
                document.getElementById('bd-available-stock').innerText = Number(b.BCH_available_stock_kg || 0).toLocaleString() + ' kg';
                document.getElementById('bd-initial-volume').innerText = Number(b.BCH_initial_volume_kg || 0).toLocaleString() + ' kg';
                document.getElementById('bd-zone-name').innerText = b.STZ_zone_name || '-';
                document.getElementById('bd-received-date').innerText = b.BCH_received_date || '-';
                document.getElementById('bd-expiry-date').innerText = b.BCH_expiry_date || '-';
                ...
                if (b.movements && b.movements.length > 0) {
                    b.movements.forEach(m => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td class="p-2.5 font-mono font-bold">${Number(m.STM_quantity_kg).toLocaleString()} kg</td>
                    ...
                    mBody.innerHTML = '<tr><td colspan="5" class="p-4 text-center text-gray-500">Chưa có thông tin luân chuyển.</td></tr>';
                }

                document.getElementById('batch-detail-modal').classList.remove('hidden');
            } else {
                alert('Không tìm thấy thông tin lô hàng!');
            }
        })
        .catch(err => {
            console.error(err);
            alert('Lỗi tải thông tin lô hàng!');
        });
}
        ...
        editForm.addEventListener('submit', function(e) {
            ...
            if (expiryVal) {
                ...
                if (exp <= now) {
                    ...
                    err.innerText = 'Lỗi thời gian thực: Ngày hết hạn (' + exp.toLocaleDateString('vi-VN') + ') không thể thuộc về quá khứ hoặc nhỏ hơn ngày hiện tại!';
